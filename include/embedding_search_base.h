...
public:
    ...
    virtual std::vector<std::pair<SimilarityType, size_t>> similarity_search(
...
protected:
    ...
    virtual bool validateDimensions(const std::vector<std::vector<RawType>> &input, std::string &error_message) = 0;
};
...
class OptimizedEmbeddingSearchBase : public EmbeddingSearchBase<VectorType, SimilarityType>
{
protected:
...
public:
    OptimizedEmbeddingSearchBase() : config_(ConfigRef::get()),
...
protected:
    ...
    {
        embedding_data.reset(static_cast<StorageType *>(
            std::aligned_alloc(config_.memory.alignmentSize,
                               total_size * sizeof(StorageType))));
