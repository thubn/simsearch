...
private:
    ...
    int binary_cosine_similarity2(const std::vector<uint64_t>& a, const std::vector<uint64_t>& b);

public:
    EmbeddingSearch();

    const std::vector<std::vector<float>>& getEmbeddings();
    const std::vector<std::vector<uint64_t>>& getBinaryEmbeddings();
    ...
    std::vector<std::pair<int, size_t>> binary_similarity_search(const std::vector<uint64_t>& query, size_t k);
};
