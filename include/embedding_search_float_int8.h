...
class EmbeddingSearchFloatInt8 : public EmbeddingSearchBase<std::vector<SimplifiedFloat>, float>
{
public:
    EmbeddingSearchFloatInt8() = default;

    std::vector<std::pair<float, size_t>> similarity_search(const std::vector<SimplifiedFloat> &query, size_t k) override;
...
private:
    float cosine_similarity(const std::vector<SimplifiedFloat> &a, const std::vector<SimplifiedFloat> &b);
};