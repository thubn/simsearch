...
public:
  PyEmbeddingSearch() {
    ...
    config_path = std::filesystem::path(py::module::import("embedding_search")
                                            .attr("__file__")
                                            .cast<std::string>())
                      .parent_path()
                      .string() +
  ...
  bool load(const std::string &filename) {
    try {
      ...
      binary_avx2_searcher =
          std::make_unique<OptimizedEmbeddingSearchBinaryAVX2>();
      uint8_avx2_searcher =
          std::make_unique<OptimizedEmbeddingSearchUint8AVX2>();
  ...
  py::tuple benchmark_search(py::array_t<float> query_vectors, size_t k,
                             size_t num_runs) {
    if (!is_initialized) {
    ...
    for (size_t q = 0; q < num_queries; q++) {
      ...
      EmbeddingUtils::convertSingleEmbeddingToAVX2(query, queryAvx2,
                                                   query.size() / 8);
      ...
      // Float search (baseline)
      {
        ...
        float_times.push_back(std::chrono::duration_cast<std::chrono::microseconds>(end
        - start).count());
        */
        ...
        auto end = std::chrono::high_resolution_clock::now();
        avx2_times.push_back(
            std::chrono::duration_cast<std::chrono::microseconds>(end - start)
                .count());
        // avx2_accuracies.push_back(EmbeddingUtils::calculateJaccardIndex(float_results,
        // avx2_results));

        // Binary AVX2 search
        start = std::chrono::high_resolution_clock::now();
        auto binary_results =
            binary_avx2_searcher->similarity_search(queryBinaryAvx2, k);
        end = std::chrono::high_resolution_clock::now();
        binary_times.push_back(
            std::chrono::duration_cast<std::chrono::microseconds>(end - start)
                .count());
        binary_accuracies.push_back(EmbeddingUtils::calculateJaccardIndex(
            avx2_results, binary_results));

        /*
        // UINT8 AVX2 search
        start = std::chrono::high_resolution_clock::now();
        auto uint8_results =
        uint8_avx2_searcher.similarity_search(uint8_avx2_searcher.floatToAvx2(query),
        k); end = std::chrono::high_resolution_clock::now();
        uint8_times.push_back(std::chrono::duration_cast<std::chrono::microseconds>(end
        - start).count());
        uint8_accuracies.push_back(EmbeddingUtils::calculateJaccardIndex(float_results,
        uint8_results));
        */
      }
    }
    ...
    return py::make_tuple(float_times, avx2_times, binary_times, uint8_times,
  ...
  py::tuple search(py::array_t<float> query_vector, size_t k,
    ...
    try {
      ...
      avx2_time = std::chrono::duration_cast<std::chrono::microseconds>(
                      avx2_end - avx2_start)
                      .count();
      ...
      binary_time = std::chrono::duration_cast<std::chrono::microseconds>(
      ...
      auto uint8_results =
          uint8_avx2_searcher->similarity_search(queryUint8Avx2, k);
      auto uint8_end = std::chrono::high_resolution_clock::now();
      uint8_time = std::chrono::duration_cast<std::chrono::microseconds>(
                       uint8_end - uint8_start)
                       .count();

      // Perform two step Binary AVX2 search with timing
      auto twostep_start = std::chrono::high_resolution_clock::now();
      auto twostep_binary_results = binary_avx2_searcher->similarity_search(
          queryBinaryAvx2, k * rescoring_factor);
      auto twostep_avx2_results =
          avx2_searcher->similarity_search(query, k, twostep_binary_results);
      auto twostep_end = std::chrono::high_resolution_clock::now();
      twostep_time = std::chrono::duration_cast<std::chrono::microseconds>(
                         twostep_end - twostep_start)
                         .count();

      // Calculate comparison metrics
      double binary_jaccard =
          EmbeddingUtils::calculateJaccardIndex(avx2_results, binary_results);
      double uint8_jaccard =
          EmbeddingUtils::calculateJaccardIndex(avx2_results, uint8_results);
      double twostep_jaccard = EmbeddingUtils::calculateJaccardIndex(
          avx2_results, twostep_avx2_results);

      double binary_ndcg =
          EmbeddingUtils::calculateNDCG(avx2_results, binary_results);
      double uint8_ndcg =
          EmbeddingUtils::calculateNDCG(avx2_results, uint8_results);
      double twostep_ndcg =
          EmbeddingUtils::calculateNDCG(avx2_results, twostep_avx2_results);

      // Get sentences
      const auto &sentences = avx2_searcher->getSentences();

      // Convert results to Python lists
      py::list avx2_results_py, binary_results_py, uint8_results_py,
          twostep_results_py;

      for (const auto &result : avx2_results) {
        avx2_results_py.append(py::make_tuple(
            result.first, result.second,
            EmbeddingUtils::sanitize_utf8(sentences[result.second])));
      }
      for (const auto &result : binary_results) {
        binary_results_py.append(py::make_tuple(
            result.first, result.second,
            EmbeddingUtils::sanitize_utf8(sentences[result.second])));
      }
      for (const auto &result : uint8_results) {
        uint8_results_py.append(py::make_tuple(
            result.first, result.second,
            EmbeddingUtils::sanitize_utf8(sentences[result.second])));
      }
      for (const auto &result : twostep_avx2_results) {
        twostep_results_py.append(py::make_tuple(
            result.first, result.second,
            EmbeddingUtils::sanitize_utf8(sentences[result.second])));
      }

      return py::make_tuple(avx2_results_py, binary_results_py,
                            uint8_results_py, twostep_results_py, avx2_time,
                            binary_time, uint8_time, twostep_time,
...
PYBIND11_MODULE(embedding_search, m) {
  ...
  py::class_<PyEmbeddingSearch>(m, "EmbeddingSearch")
      ...
      .def("search", &PyEmbeddingSearch::search,
           "Perform similarity search with all methods")
      .def("benchmark_search", &PyEmbeddingSearch::benchmark_search,
           "Benchmark similarity search with multiple queries");
}